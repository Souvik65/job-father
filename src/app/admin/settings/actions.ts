...
export async function changeAdminPassword(
  ...
  if (!user?.password) {
    return { ok: false, message: 'Cannot update password for this account type.' };
  }
  ...
  if (!/[A-Z]/.test(newPassword)) {
    return { ok: false, message: 'New password must include at least one uppercase letter.' };
  }
  if (!/[a-z]/.test(newPassword)) {
    return { ok: false, message: 'New password must include at least one lowercase letter.' };
  }
  if (!/[0-9]/.test(newPassword)) {
    return { ok: false, message: 'New password must include at least one number.' };
  }
  if (!/[^A-Za-z0-9]/.test(newPassword)) {
    return { ok: false, message: 'New password must include at least one special character.' };
  }
...
export async function saveFrontendSettings(
  ...
  if (popupDelay < 1 || popupDelay > 300) {
    return { ok: false, message: 'Popup delay must be between 1 and 300 seconds.' };
  }
