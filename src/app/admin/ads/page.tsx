      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="bg-white border border-[#c6c5d4] rounded-xl p-4 flex flex-col gap-1 shadow-sm">
          <div className="flex items-center gap-2 text-[#454652]">
            <span
              className="material-symbols-outlined text-[#5aa958]"
              style={{ fontSize: 20 }}
            >
              campaign
            </span>
        ...
        <div className="bg-white border border-[#c6c5d4] rounded-xl p-4 flex flex-col gap-1 shadow-sm">
          <div className="flex items-center gap-2 text-[#454652]">
            <span
              className="material-symbols-outlined text-[#343d96]"
              style={{ fontSize: 20 }}
            >
              visibility
            </span>
        ...
        <div className="bg-white border border-[#c6c5d4] rounded-xl p-4 flex flex-col gap-1 shadow-sm col-span-2 md:col-span-1">
          <div className="flex items-center gap-2 text-[#454652]">
            <span
              className="material-symbols-outlined text-[#fc820c]"
              style={{ fontSize: 20 }}
            >
              ads_click
            </span>
      ...
      <div className="bg-white border border-[#c6c5d4] rounded-xl overflow-hidden shadow-sm">
        ...
        <div className="border-b border-[#c6c5d4] bg-[#f3f3f3] px-4 py-3 flex items-center justify-between gap-3">
          <span className="text-sm font-bold text-[#000666]">
            All Campaigns
            <span className="ml-2 text-xs font-normal text-[#454652]">
              ({ads.length} total)
            </span>
          </span>
          <Link
            ...
            <tbody className="divide-y divide-[#e8e8e8]">
              {ads.length === 0 ? (
                <tr>
                  <td
                    colSpan={7}
                    className="py-16 text-center text-[#454652] text-sm"
                  >
                ...
                ads.map((ad) => {
                  const detail = POSITION_DETAILS[ad.position];
                  const pos = detail
                    ? { label: detail.shortLabel, icon: detail.icon }
                    : { label: ad.position, icon: "help_outline" };
                  return (
                    <tr
                      key={ad.id}
                      className={`hover:bg-[#f9f9f9] transition-colors group ${!ad.isActive ? "opacity-60" : ""}`}
                    >
                      {/* Preview thumbnail */}
                          ...
                          {ad.imageUrl ? (
                            <>
                              ...
                              <img
                                src={ad.imageUrl}
                                alt={ad.label}
                                className={`absolute inset-0 w-full h-full object-cover ${!ad.isActive ? "grayscale" : ""}`}
                              />
                      ...
                      <td className="py-4 px-4">
                        <div
                          className={`font-bold text-sm mb-0.5 ${ad.isActive ? "text-[#000666]" : "text-[#454652] line-through"}`}
                        >
                          {ad.label}
                        </div>
                        <div className="text-xs text-[#767683] flex items-center gap-1">
                          ...
                          {ad.type === AdType.GOOGLE
                            ? "AdSense Managed"
                            : (ad.targetUrl ?? "No target URL")}
                        </div>
                        {ad.notes && (
                          <div
                            className="text-xs text-[#767683] mt-1 italic truncate max-w-[200px]"
                            title={ad.notes}
                          >
                            {" "}
                            {ad.notes}
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-1.5 text-sm text-[#1a1c1c]">
                          <span
                            className="material-symbols-outlined text-[#767683]"
                            style={{ fontSize: 16 }}
                          >
                            {pos.icon}
                          </span>
                      ...
                      <td className="py-4 px-4 text-center">
                        ...
                        >
                          ...
                          >
                            ...
                            <span
                              className={`text-[10px] font-bold ${ad.isActive ? "text-[#5aa958]" : "text-[#454652]"}`}
                            >
        ...
        {ads.length > 0 && (
          <div className="bg-[#f3f3f3] border-t border-[#c6c5d4] px-4 py-3">
            <span className="text-sm text-[#454652]">
              <span className="font-semibold text-[#1a1c1c]">{ads.length}</span>{" "}
              campaign{ads.length !== 1 ? "s" : ""} total ·{" "}
              <span className="font-semibold text-[#5aa958]">
                {totalActive} active
              </span>
            </span>
          </div>
