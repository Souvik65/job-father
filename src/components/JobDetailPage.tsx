...
export function JobDetailPage({ job, portalName = 'Jobfather' }: JobDetailPageProps) {
  const [shareOpen, setShareOpen] = useState(false);
  const [copied, setCopied] = useState(false);

  const shareText = buildShareText(job);
  const shareUrl = typeof window !== 'undefined' ? `${window.location.origin}/job/${job.slug}` : jobUrl(job);
    ...
    <div className="flex flex-col min-h-screen bg-[#f9f9f9] dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-colors duration-300">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-3 sm:px-4 py-5 sm:py-8">
        ...
        <div className="mb-4 sm:mb-6">
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest text-gray-500 hover:text-[#ff7315] dark:text-slate-400 dark:hover:text-[#ff7315] transition-colors cursor-pointer select-none"
          >
        ...
        <div className="w-full flex flex-col-reverse md:flex-row gap-4 sm:gap-6 items-start">
          ...
          <div className="w-full md:w-2/3 flex flex-col gap-6">
            ...
            <section className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-4 sm:p-6 flex flex-col gap-3 shadow-sm">
              ...
              <h1 className="text-xl sm:text-2xl font-black text-[#000666] dark:text-indigo-400 leading-tight">{job.title}</h1>
              {job.organization && (
                <p className="text-sm text-gray-500 dark:text-slate-450 flex items-center gap-2">
                  ...
                  <span className="font-medium text-gray-700 dark:text-slate-300">{job.organization}</span>
                </p>
              )}
              {job.description && (
                <p className="text-sm text-gray-600 dark:text-slate-350 whitespace-pre-wrap leading-relaxed border-t border-gray-100 dark:border-slate-800 pt-4 mt-2">{job.description}</p>
              )}
            </section>
            ...
            {(appStart || appEnd || examDate) && (
              <section className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-6 flex flex-col gap-4 shadow-sm">
                <h2 className="text-lg font-bold text-gray-900 dark:text-slate-100 border-b border-gray-100 dark:border-slate-800 pb-3">Important Dates</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {appStart && (
                    <DateCard
                      icon={
                        <svg className="w-5 h-5 text-[#000666] dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {job.totalVacancies && (
              <section className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-6 flex flex-col gap-4 shadow-sm">
                <h2 className="text-lg font-bold text-gray-900 dark:text-slate-100 border-b border-gray-100 dark:border-slate-800 pb-3">Vacancy Details</h2>
                <div className="flex items-center gap-4 bg-[#f3f3f3] dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-4">
                  <svg className="w-6 h-6 text-[#000666] dark:text-indigo-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
                  </svg>
                  <div>
                    <p className="text-xs font-black text-gray-400 dark:text-slate-450 uppercase tracking-widest">Total Posts</p>
                    <p className="text-lg font-black text-gray-900 dark:text-slate-100">{job.totalVacancies}</p>
                  </div>
                </div>
              </section>
            )}
          ...
          <div className="w-full md:w-1/3 flex flex-col gap-6 md:sticky md:top-24">
            ...
            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-6 flex flex-col gap-4 shadow-sm">
              <div className="pb-4 border-b border-gray-100 dark:border-slate-800 flex flex-col gap-1">
                <span className="text-xs font-black text-gray-400 dark:text-slate-450 uppercase tracking-widest">Application Deadline</span>
            ...
            <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/40 rounded-2xl p-5 flex gap-3 shadow-inner">
              <svg className="w-5 h-5 text-[#000666] dark:text-indigo-400 shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 24 24">
